 ...
 * NodePointer structs, one for each device used in the voxelization. Functions 
 * that say that they return host pointers only return a single \p NodePointer, 
 * due to multidevice voxelization being disabled in that case.
...
private:
    ...
    /// Options or state-tracking variables.
    struct Options
    {
        ...
        bool voxelDistanceGiven;   /**< \p True if the user has specified the 
                                        distance between voxels. */
        bool simulateMultidevice;  /**< Simulates multiple devices on one 
                                        device. */
    };

    Options options;               ///< \p Options for the voxelization.

    std::ofstream log;             ///< Writes to file is printing is enabled.

    bool fatalError;               ///< Allows immediate termination.
    clock_t startTime;             ///< Time taken at start of voxelization.

    void initVariables();          ///< Initializes variables upon contruction.
    ...
    /// Initializes the \p Options struct with sensible default values.
    void initOptions();
    ...
    void fccWorker( uint  xRes
                  ...
                  , uint3 matSplitRes
                  , DevContext<Node> & device );
    /// Wraps \p cudaMemcpy() with some reporting and error handling.
    void devMemcpy( void* dest
                  , void const * src
                  , size_t count
                  , cudaMemcpyKind kind
                  , std::string what );
    /// Wraps \p cudaMemset() with some reporting and error handling.
    void devMemset( void* devPtr, int value, size_t count, std::string what );
...
public:
    ...
    NodePointer<Node> 
        voxelizeSliceToRAM( double cubeLength
                          ...
                          , uint2 voxSplitRes = make_uint2( 1024, 512 )
                          , uint2 matSplitRes = make_uint2( 512, 512 ) );
    /// Returns the distance between voxel centers.
    /** \return Distance between voxel centers. */
    double getVoxelLength() const { return this->hostVars.voxelLength; }
    /// Returns the minimum corner of the model's bounding box.
    /** \return Minimum corner of the model's bounding box. */
    double3 getMinVertex() const { return this->hostVars.minVertex; }
    /// Returns the maximum corner of the model's bounding box.
    /** \return Maximum corner of the model's bounding box. */
    double3 getMaxVertex() const { return this->hostVars.maxVertex; }
    /// Returns the dimensions of the allocated voxels or \p Nodes.
    /** \return Dimensions of the allocated voxel or \p Node array. */
    uint3 getResolution() const { return this->hostVars.resolution.max; }
    /// Returns the vertices of the model.
    /** \return Vertices of the model. */
    std::vector<float> getVertices() const { return this->hostVars.vertices; }
    /// Returns the indices of the model.
    /** \return Indices of the model. */
    std::vector<uint> getIndices() const { return this->hostVars.indices; }
    /// Returns the number of triangles in the model.
    /** \return Number of triangles in the model. */
    uint getNrOfTriangles() const { return this->hostVars.nrOfTriangles; }
    ...
    void setMaterialOutput( bool _materials );
